...
class RadiaCode:
    ...
    async def _init_device(self, ignore_firmware_compatibility_check = False):
        ...
        lines = await self.configuration()
    ...
    async def execute(self, reqtype: bytes, args: Optional[bytes] = None) -> BytesBuffer:
    ...
    async def read_request(self, command_id: Union[int, VS, VSFR]) -> BytesBuffer:
    ...
    async def write_request(self, command_id: Union[int, VSFR], data: Optional[bytes] = None) -> None:
        r = await self.execute(b'\x25\x08', struct.pack('<I', int(command_id)) + (data or b''))
        retcode = r.unpack('<I')[0]
        assert retcode == 1
        assert r.size() == 0

    async def batch_read_vsfrs(self, vsfr_ids: List[VSFR]) -> List[int]:
        assert len(vsfr_ids)
        r = await self.execute(b'\x2a\x08', b''.join(struct.pack('<I', int(c)) for c in vsfr_ids))
        ret = [r.unpack('<I')[0] for _ in range(len(vsfr_ids))]
        assert r.size() == 0
        return ret

    async def status(self) -> str:
        r = await self.execute(b'\x05\x00')
        flags = r.unpack('<I')
        assert r.size() == 0
    ...
    async def set_local_time(self, dt: datetime.datetime) -> None:
        ...
        await self.execute(b'\x04\x0a', d)

    async def fw_signature(self) -> str:
        r = await self.execute(b'\x01\x01')
    ...
    async def fw_version(self) -> tuple[tuple[int, int, str], tuple[int, int, str]]:
        r = await self.execute(b'\x0a\x00')
    ...
    async def hw_serial_number(self) -> str:
        r = await self.execute(b'\x0b\x00')
    ...
    async def serial_number(self) -> str:
        r = await self.read_request(8)
        return r.data().decode('ascii')

    async def commands(self) -> str:
        br = await self.read_request(257)
    ...
    async def data_buf(self) -> List[Union[DoseRateDB, RareData, RealTimeData, RawData, Event]]:
    ...
    async def spectrum_reset(self) -> None:
        r = await self.execute(b'\x27\x08', struct.pack('<II', int(VS.SPECTRUM), 0))
        retcode = r.unpack('<I')[0]
        assert retcode == 1
        assert r.size() == 0
    ...
    async def set_energy_calib(self, coef: List[float]) -> None:
        ...
        r = await self.execute(b'\x27\x08', struct.pack('<II', int(VS.ENERGY_CALIB), len(pc)) + pc)
        retcode = r.unpack('<I')[0]
        assert retcode == 1

    async def set_language(self, lang='en') -> None:
    ...
    async def set_device_on(self, on: bool):
